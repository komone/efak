...
%% @doc	Handles permanent tcp connection created by {@link tcp_ch_connector}.
%% 		This connection does not times out.
%% @end
-module(tcp_ch_connection).
...
%% @end
%%--------------------------------------------------------------------
disconnected({connected, Socket}, State) ->
    {ok, {IP, _}} = inet:peername(Socket),
    inet:setopts(Socket, [{active, once}]),
    {next_state, connected, State#state{socket=Socket, addr=IP}};
...
%%
%% @doc     line_down requests are ignored.
%% @hidden
%% @end
%%--------------------------------------------------------------------
...
%% @end
%%--------------------------------------------------------------------
connected({send, Bin}, #state{socket=Sock} = State) ->
...
handle_info({tcp, Socket, Bin}, connected,
			#state{interface=Name} = StateData) when is_binary(Bin) ->
...
initialize_state(Params) ->
	...
	#state {
		interface		= Interface,
		msg				= #imsg{src={Interface,self()},msg=undefined},
		ip				= Host,
		port			= Port,
		retry			= Retry,
		call_delay		= Delay,
...
%% @end
%%--------------------------------------------------------------------
do_connect(#state{retry=R, ip=Host, port=Port, tcp_opt=Opt, msg=undefined} = State) ->
	case gen_tcp:connect(Host, Port, Opt) of
		{ok, Sock} -> 
			{ok, {IP, _}} = inet:peername(Sock),
			inet:setopts(Sock, [{active, once}]),
			{next_state, connected, State#state{socket=Sock, addr=IP}};
		{error, _Reason} ->
...
%% @end
%%--------------------------------------------------------------------
do_connect(#state{retry=R, ip=Host, port=Port, tcp_opt=Opt, msg=Bin} = State) ->
	case gen_tcp:connect(Host, Port, Opt) of
		{ok, Sock} -> 
			{ok, {IP, _}} = inet:peername(Sock),
			ok = gen_tcp:send(Sock, Bin),
