...
%% @doc	Establishes permanent outgoing tcp connections.
...
%%		to work.
%%
%%		The channel connector handles permanent tcp connections.
%% @see tcp_ch_connection
%% @end

-module(tcp_ch_connector).
...
%%		<li>`{max_conn, Max}'		Max number of spawned connections</li>
...
%%			line_proxy calls this function when a new connection is
%%			required.
%%
%%			If the max number of allowed connections is reached a
%%			connection is reused to send the message.
%% @end	
%%--------------------------------------------------------------------
...
%%			A new connection is spawned only if the max number of
%% 			allowed connection is not reached.
%% @hidden
%% @end
%%--------------------------------------------------------------------
connecting({connect, Msg},
			 #state{module=Module, max_conn=MaxConn,
			 connections=Conns, conn_sup_name=SupName, conn_opt=Params} = State) ->
	case length(Conns) of
		NConns when NConns >= MaxConn ->
			Pid = lists:nth(erlang:phash2(now(),NConns)+1, Conns),
			Module:send(Pid, Msg),
			NewConns = Conns;
		_ ->
    ...
    {next_state, connecting, State#state{connections=NewConns}};
...
%% @end
%%--------------------------------------------------------------------
handle_info({'EXIT', Pid, _Reason}, StateName, #state{connections=Conns} = State) ->
	NewConns = lists:delete(Pid, Conns),
    {next_state, StateName, State#state{connections=NewConns}};
...
initialize_state(Params) ->
	...
	MaxConn = config_server:get_attr_value(max_conn, Params),
	ConnectionSupName = list_to_atom(lists:flatten([Interface, "_conn_sup"])),
	Options = lists:subtract(Params, [{module,Module}, {max_conn,MaxConn}]),
	#state {
		interface		= list_to_atom(Interface),
		module 			= list_to_atom(Module),
		max_conn 		= list_to_integer(MaxConn),
